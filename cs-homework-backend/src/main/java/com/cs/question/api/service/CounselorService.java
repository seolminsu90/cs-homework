...
public class CounselorService {
	...
	public CounselorService(CounselorRepository counselorRepository, QuestionRepository questionRepository,
			QuestionReceiveRepository questReceiveRepository) {
	...
	public List<QuestionReceive> getQuestionReceive(String id) {
		return questReceiveRepository.findByCounselorIdOrderByCheckStateDesc(id);
	}

	public QuestionReceive receiveQuestion(String id, Long questionId) {
		Question question = questionRepository.findById(questionId)
				.orElseThrow(() -> new CustomException(1404, "문의를 찾을 수 없습니다."));
	...
	public QuestionReceive writeResponse(QuestionReceiveDTO questionReceiveDTO, String id, Long questionId) {
		QuestionReceive saveReponse = questReceiveRepository.findByQuestionSeqAndCounselorId(questionId, id)
				.orElseThrow(() -> new CustomException(1406, "문의가 없거나 상담사에게 할당된 문의가 아닙니다."));
